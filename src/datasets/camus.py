...
class CamusDataset:
    ...
    def get_dataloaders(cls, cfg):
        """
        Creates DataLoaders for training, validation, and testing for CAMUS.
        """
        logger = get_logger()
        ...
        ids_tr = cls._read_ids(os.path.join(split_dir, "subgroup_training.txt"))
        ids_val = cls._read_ids(os.path.join(split_dir, "subgroup_validation.txt"))
        ids_ts = cls._read_ids(os.path.join(split_dir, "subgroup_testing.txt"))
        
        logger.info(f"IDs found -> train {len(ids_tr)} · val {len(ids_val)} · test {len(ids_ts)}")

        train_files = cls._get_files(ids_tr, nii_dir)
        val_files = cls._get_files(ids_val, nii_dir)
        test_files = cls._get_files(ids_ts, nii_dir)
