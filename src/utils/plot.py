...
def corr_and_ba(ref, pred):
    """Ref/Pred correlation and Bland-Altman statistics."""
...
def dice_reliability_curve(dice_values, n_points=101):
    """Return thresholds and coverage P(Dice >= t)."""
...
def plot_metrics_summary(df, save_path=None):
    """Plots Median Dice and HD95 with IQRs (Panel a-d)."""
    setup_style()
    structures = ["LV", "MYO", "LA"]
    
    # 1. Calculate medians and errors
    dice_medians, dice_err = [], []
    hd_medians,   hd_err   = [], []
    
    dice_data = [] # For boxplots
    hd_data   = [] # For boxplots
    ...
    # (b) Median HD95 per structure with IQR
...
def plot_clinical_bland_altman(df_ef, save_path=None):
    """Plots Correlation and Bland-Altman for ED, ES, EF (Panel a-f)."""
    setup_style()
...
def plot_reliability_curves(df_metrics, df_ef, save_path=None):
    """Plots Dice and EF reliability curves (Panel a-d)."""
    setup_style()
...
# =========================================================
#  QUALITATIVE PLOTS (Takes Images/Masks)
# =========================================================
...
def plot_qualitative_grid(samples, save_path=None):
    """Plots rows of samples (Image+Contour | Error Map | GT Labels)."""
    setup_style()
    n_rows = len(samples)
...
def plot_ef_category_roc(df_ef, save_path=None):
    """
    (Cell 8) Plots Confusion Matrix and ROC for Low EF detection.
    """
    setup_style()
    # Define Categories
    ...
    scores = 45.0 - df_ef["EF_pred"] # Invert score: lower EF = higher risk
...
def plot_results_table(df_data, title, save_path=None):
    """
    (Cells 13-17) Generic function to render a DataFrame as a high-res figure table.
    """
    setup_style()
